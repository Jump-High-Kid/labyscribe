...
**로드맵 현황**: Phase 0(추출 코어)·Phase 1(`parse_vtt`)·Phase 2(MCP stdio 서버) **완료** · Phase 3(저장 안전)~Phase 6(E2E) 대기.

## 명령어

- 추출 CLI: `python extract.py <youtube-url> --out <dir>` (`--lang ko,en` 선택 · 미지정 시 원어 자동감지·우선)
...
- 산출물: `<dir>/transcript.txt`(10분 마커) · `<dir>/raw/<id>.<lang>.vtt`(원본 불변) · `<dir>/meta.json`(`orig_lang`·`translated`·`lang`·`status`)
...
`0` OK · `2` NO_SUBTITLE · `3` DOWNLOAD_FAILED(재시도성) · `4` UNAVAILABLE · `5` BAD_INPUT · `6` EMPTY_TRANSCRIPT
...
- 핸들 = 프로세스 생애 인메모리(재시작=무효). 재조회·불변 저장·자원 총량은 Phase 3.

## 개발 규율

- `.harness/` **하네스 워크플로우**로 개발(토의→기획→실행→검증→마무리) · **codex 교차감사** 반영
- **Phase 경계 엄수**: 각 Phase 설계의도에 '의도적 제외' 목록이 있다 — 저장 원자성 = Phase 3 · 보안경화 = Phase 4 · 패키징(PyInstaller/`.mcpb`) = Phase 5. 다음 Phase 항목을 범위로 당겨오지 말 것.
